...
import type { CarTelemetryData } from '../../types/parserTypes'
...
export class CarTelemetryDataParser extends F1Parser<CarTelemetryData> {
  constructor(packetFormat: number) {
    ...
    this.uint8('m_clutch').int8('m_gear').uint16le('m_engineRPM').uint8('m_drs').uint8('m_revLightsPercent')
    ...
    this.array('m_brakesTemperature', {
      ...
      .array('m_tyresSurfaceTemperature', {
        length: 4,
        type: packetFormat >= 2020 ? new Parser().uint8('') : new Parser().uint16le(''),
      })
